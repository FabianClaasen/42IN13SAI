	...
	while (lineRemaining.length() != 0)
	{
		...
		for (definitionIterator = tokenDefinitions.begin(); definitionIterator != tokenDefinitions.end(); ++definitionIterator)
		{
			...
			if (matched > 0)
			{
				...
				if (definition.tokenType == TokenType::CloseCurlyBracket ||
					definition.tokenType == TokenType::CloseMethod ||
					definition.tokenType == TokenType::Else || definition.tokenType == TokenType::ElseIf ||
					definition.tokenType == TokenType::While)
				{
					try {
